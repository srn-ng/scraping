...
with open('GoMyCode.csv', 'w', newline='', encoding='utf-8') as csvfile:
    fieldnames = ['Catégorie','URL', 'Titre', 'Prix', 'Date of scraping']
    ...
    for index, url in enumerate(urls, 1):   
        ...
        if response.status_code == 200: 
            ...
            for element in elements:
                ...
                ######################################################################################
                formations_info.append({  'URL':url ,'Titre': title, 'Prix': price, 'Date of scraping': today_date})

    
            writer.writerow({'Catégorie': f'Catégorie {index}','URL': '', 'Titre': '', 'Prix': '', 'Date of scraping': ''})
